# Final Project - OIM3640
Team members: Finn, Kalib

# Overview
We are building a multi‑game web application using Python and Flask.  
The backend handles game logic (starting with Snake), and the frontend provides a clean, responsive interface with a shared navigation layout.

# Current Games
- Snake (engine by Finn, UI by Kalib)
- Rubik’s Timer (UI by Kalib)
- Tetris Prototype (planned)

# Tech Stack
- Python (game engines + Flask API)
- HTML/CSS/JavaScript (front-end)
- Responsive UI with hamburger menu + slide‑out navigation

<!-- Run python app.py -->